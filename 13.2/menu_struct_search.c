...
int main()
{
    ...
    if (MAX > 30)
    {
        puts("Melebihi jumlah maksimal data");
        exit(0);
    }
    menu();
...
void sortShell(mhs A[], int opsi)
{
    ...
    while (jarak > 1)
    {
        ...
        while (didSwap)
        {
            ...
            {
                compare = (opsi == 1) ? (A[i].no > A[i + jarak].no) : (strcmp(A[i].nama, A[i + jarak].nama) > 0);
                if (compare)
                {
...
{
    int l = 0, r = MAX - 1, found = 0, index = 0;
